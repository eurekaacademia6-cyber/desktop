...
class Candle:
    ...
    def upper_wick_px(self) -> float:
        return max(0.0, min(self.open_px, self.close_px) - self.high)

    @property
    def lower_wick_px(self) -> float:
        return max(0.0, self.low - max(self.open_px, self.close_px))
...
class Detection:
    ...
    def usable(self) -> bool:
        return len(self.candles) >= 10 and self.quality >= 0.60
